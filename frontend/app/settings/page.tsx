...
const TIER_MAP: Record<string, string> = {
  MIT: "T5", Stanford: "T5", CMU: "T5", Berkeley: "T5", Harvard: "T5",
  Princeton: "T10", "University of Washington": "T10", Cornell: "T10",
  Columbia: "T20", NYU: "T20",
};
...
const SUGGESTED_INTERESTS = [
  "NLP", "LLM Alignment", "Computer Vision", "Reinforcement Learning",
  "Robotics", "ML Systems", "Drug Discovery", "Computational Biology",
...
function TagInput({
  ...
  return (
    <div>
      <label className="block text-[10px] font-bold uppercase tracking-widest font-space mb-2" style={{ color: "#9CA3AF" }}>
        {label}
      </label>
      <div
      ...
      >
        {tags.map(tag => (
          ...
          >
            {tag}
            <button onClick={() => onRemove(tag)} style={{ color: "rgba(255,255,255,0.6)", lineHeight: 1 }}>×</button>
          </span>
        ))}
        <input
          id={`input-${label}`}
...
/* ── Country picker ── */
function CountryPicker({ selected, onAdd, onRemove }: {
  selected: string[]; onAdd: (name: string) => void; onRemove: (name: string) => void;
}) {
  ...
  const filtered = ALL_COUNTRIES
    .filter(c => !selected.includes(c.name) && c.name.toLowerCase().includes(query.toLowerCase()))
    .slice(0, 8);
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-3">
          {selected.map(name => {
            ...
            return (
              <span key={name} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold font-space"
                style={{ background: "#0D0D0D", color: "#FFFFFF", borderRadius: "4px" }}>
                {c?.flag} {name}
                <button onClick={() => onRemove(name)} style={{ color: "rgba(255,255,255,0.55)", lineHeight: 1 }}>×</button>
              </span>
            );
          })}
        </div>
      ...
      {open && filtered.length > 0 && (
        <div
          style={{
            ...
            background: "#FFFFFF", border: "2px solid #0D0D0D", borderRadius: "4px",
            boxShadow: "4px 4px 0 #0D0D0D", zIndex: 9999,
            maxHeight: "170px", overflowY: "auto",
          }}
        >
          {filtered.map(c => (
            <button key={c.code}
              onMouseDown={e => { e.preventDefault(); onAdd(c.name); setQuery(""); setOpen(false); }}
...
/* ── University picker ── */
interface UniResult { name: string; country: string; }

function UniversityPicker({ selected, onAdd, onRemove }: {
  selected: string[]; onAdd: (name: string) => void; onRemove: (name: string) => void;
}) {
  const [query, setQuery]     = useState("");
  ...
  return (
    <div>
          ...
          {selected.map(name => {
            ...
            return (
              <span key={name} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold font-space"
                style={{ background: "#0D0D0D", color: "#FFFFFF", borderRadius: "4px" }}>
                {ts && (
                  <span className="text-[9px] font-bold px-1 py-0.5" style={{ background: ts.bg, color: ts.color, borderRadius: "3px" }}>
                    {tier}
                  </span>
                )}
                {name}
      ...
      <div ref={inputRef}
        ...
        style={{ background: "#FFFFFF", border: "2px solid #0D0D0D", borderRadius: "4px" }}
      >
        <Icon icon={loading ? "solar:refresh-bold" : "solar:magnifer-bold"} width={13} style={{ color: "#9CA3AF", flexShrink: 0 }} />
        <input
          value={query}
      ...
      {open && results.length > 0 && (
        ...
        >
          {results.map(u => {
            ...
            return (
              <button key={u.name}
                onMouseDown={e => { e.preventDefault(); onAdd(u.name); setQuery(""); setResults([]); setOpen(false); }}
                className="w-full flex items-center gap-2 px-3 py-2 text-xs font-dm text-left bouncy"
                style={{ color: "#0D0D0D" }}
                onMouseEnter={e => (e.currentTarget.style.background = "#F7F0E3")}
                onMouseLeave={e => (e.currentTarget.style.background = "")}
              >
                {ts && (
                  <span className="text-[9px] font-bold px-1 py-0.5 shrink-0" style={{ background: ts.bg, color: ts.color, borderRadius: "3px" }}>
                    {tier}
                  </span>
                )}
                <span className="truncate">{u.name}</span>
                <span className="ml-auto shrink-0" style={{ color: "#9CA3AF" }}>{u.country}</span>
              </button>
            );
          })}
        </div>
...
export default function SettingsPage() {
  ...
  return (
    <div style={{ background: "#F7F0E3" }}>
      {/* Header */}
      <div className="px-6 py-4 sticky top-0 z-10" style={{ background: "#0D0D0D", borderBottom: "2px solid #E8472A" }}>
      ...
      <div className="p-6">
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          {/* Research Interests */}
          <div className="p-5" style={{ background: "#FFFFFF", border: "2px solid #0D0D0D", boxShadow: "4px 4px 0 #0D0D0D", borderRadius: "4px" }}>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-7 h-7 flex items-center justify-center" style={{ background: "#E8472A", border: "2px solid #0D0D0D", borderRadius: "4px" }}>
                <Icon icon="solar:cpu-bolt-bold" width={13} style={{ color: "#FFFFFF" }} />
              </div>
              <h2 className="text-sm font-bold font-space" style={{ color: "#0D0D0D" }}>Research Interests</h2>
            </div>
            <TagInput
          ...
          <div className="p-5" style={{ background: "#FFFFFF", border: "2px solid #0D0D0D", boxShadow: "4px 4px 0 #0D0D0D", borderRadius: "4px" }}>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-7 h-7 flex items-center justify-center" style={{ background: "#4ECDC4", border: "2px solid #0D0D0D", borderRadius: "4px" }}>
                <Icon icon="solar:buildings-bold" width={13} style={{ color: "#0D0D0D" }} />
              </div>
              <h2 className="text-sm font-bold font-space" style={{ color: "#0D0D0D" }}>Universities of Interest</h2>
            </div>
            <UniversityPicker
          ...
          <div className="p-5" style={{ background: "#FFFFFF", border: "2px solid #0D0D0D", boxShadow: "4px 4px 0 #0D0D0D", borderRadius: "4px" }}>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-7 h-7 flex items-center justify-center" style={{ background: "#0D0D0D", border: "2px solid #0D0D0D", borderRadius: "4px" }}>
                <Icon icon="solar:global-bold" width={13} style={{ color: "#FFFFFF" }} />
              </div>
              <h2 className="text-sm font-bold font-space" style={{ color: "#0D0D0D" }}>Countries of Interest</h2>
            </div>
            <CountryPicker
              selected={countries}
