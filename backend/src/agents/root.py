...
root_agent = LlmAgent(
    ...
    instruction=(
        ...
        "shortlist faculty, creating/updating a tracker application, adding a recommender, "
        "saving/editing a draft, drafting/sending an email, or changing a NAMED profile/preferences "
        "field to a specific value (e.g. 'add Germany to my target countries', 'set my degree to PhD') — "
        "goes to the internal application agent. It is the ONLY branch that owns those writes "
        "and the human-approval (HITL) gate. Do NOT route soft/aspirational or 'remember this' "
        "statements here (see DISCLOSURE above) — only explicit, specific changes.\n"
