...
Context Engineering: Sessions, MemoryNovember 2025Table of contents
...
Context Engineering: Sessions, MemoryNovember 2025•  Memory: The mechanism for long-term persistence, capturing and consolidating key
...
Context Engineering: Sessions, MemoryNovember 2025tools, and a clear understanding of the presentation style, they can reliably produce an
...
Context Engineering: Sessions, MemoryNovember 2025•  Artifacts: Non-textual data (e.g., files, images) associated with the user or session.
...
Context Engineering: Sessions, MemoryNovember 2025This practice manifests as a continuous cycle within the agent's operational loop for each
...
Context Engineering: Sessions, MemoryNovember 20254. Upload Context: New information gathered during the turn is uploaded to persistent
...
Context Engineering: Sessions, MemoryNovember 2025Sessions
...
Context Engineering: Sessions, MemoryNovember 2025Python
...
Context Engineering: Sessions, MemoryNovember 2025Agent frameworks act as a universal translator between your code and a LLM. While you,
...
Context Engineering: Sessions, MemoryNovember 2025
...
Context Engineering: Sessions, MemoryNovember 2025Figure 3: Different multi-agent architectural patterns30
...
Context Engineering: Sessions, MemoryNovember 2025Agent frameworks handle session history for multi-agent systems using one of two primary
...
Context Engineering: Sessions, MemoryNovember 2025# Parent agent.
root_agent = LlmAgent(
  ...,
...
Context Engineering: Sessions, MemoryNovember 2025One emerging architectural pattern architectural pattern for coordinating collaboration
...
Context Engineering: Sessions, MemoryNovember 2025Security and Privacy
...
Context Engineering: Sessions, MemoryNovember 2025Performance and Scalability
...
Context Engineering: Sessions, MemoryNovember 20253.  Latency (Speed): Sending more text to the model takes longer to process, resulting
...
Context Engineering: Sessions, MemoryNovember 2025•  Token-Based Truncation: Before sending the history to the model, the agent counts the
...
Context Engineering: Sessions, MemoryNovember 2025Memory
...
Context Engineering: Sessions, MemoryNovember 2025•  Context Window Management: As conversations become longer, the full history can
...
Context Engineering: Sessions, MemoryNovember 20253.  The Agent Framework (e.g., ADK, LangGraph): Provides the structure and tools for
...
Context Engineering: Sessions, MemoryNovember 2025Figure 5: The flow of information between sessions, memory, and external knowledge
...
Context Engineering: Sessions, MemoryNovember 2025RAG Engines

Memory Managers

Primary Goal

To inject external, factual knowledge into
the context

Data source

A static, pre-indexed external knowledge
base (e.g., PDFs, wikis, documents, APIs).

Isolation Level

Generally Shared. The knowledge
...
RAG data is almost always retrieved “as-
...
A natural language snippet or a
structured profile.
...
Context Engineering: Sessions, MemoryNovember 2025Types of memory
...
Context Engineering: Sessions, MemoryNovember 2025Types of information
...
Context Engineering: Sessions, MemoryNovember 2025The collections13 pattern organizes content into multiple self-contained, natural language
...
Context Engineering: Sessions, MemoryNovember 2025Vector databases are the most common approach, enabling retrieval based on semantic
...
Context Engineering: Sessions, MemoryNovember 2025Memories can also be distinguished by whether the memory extraction logic is located
...
Context Engineering: Sessions, MemoryNovember 2025token-heavy transcript with a concise set of key facts. Crucially, this memory is distinct
...
Context Engineering: Sessions, MemoryNovember 2025Memory with Multimodal Content is a more advanced approach where the memory
...
Context Engineering: Sessions, MemoryNovember 2025The memory manager's LLM decides what to extract by following a carefully constructed
...
Context Engineering: Sessions, MemoryNovember 2025•  Memory Relevance Decay: Not all memories remain useful forever. An agent must
...
Context Engineering: Sessions, MemoryNovember 2025Memory Provenance
...
Context Engineering: Sessions, MemoryNovember 2025other, the agent must use this hierarchy in a conflict resolution strategy. Common strategies
...
Context Engineering: Sessions, MemoryNovember 2025Accounting for memory lineage during inference
...
Context Engineering: Sessions, MemoryNovember 2025•  Turn Cadence: Running the process after a specific number of turns (e.g., every 5 turns).
...
Context Engineering: Sessions, MemoryNovember 2025Memory retrieval searches for the most pertinent memories for the current conversation. An
...
Context Engineering: Sessions, MemoryNovember 2025With query rewriting, an LLM can be used to improve the search query itself. This can
...
Context Engineering: Sessions, MemoryNovember 2025and applications are fundamentally different. Fine-tuning is a relatively slow, offline training
...
Context Engineering: Sessions, MemoryNovember 2025Memory retrieval performance metrics evaluate the agent's ability to find the right memory
...
Context Engineering: Sessions, MemoryNovember 2025Production considerations for Memory
...
Context Engineering: Sessions, MemoryNovember 2025This service-based, non-blocking approach ensures that failures or latency in the memory
...
Context Engineering: Sessions, MemoryNovember 2025Privacy and security risks
...
Context Engineering: Sessions, MemoryNovember 2025Conclusion
...
Context Engineering: Sessions, MemoryNovember 2025Endnotes
...
Context Engineering: Sessions, MemoryNovember 202522. https://cloud.google.com/vertex-ai/generative-ai/docs/agent-engine/memory-bank/
